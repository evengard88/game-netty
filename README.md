...
Данная версия использует быстрый в реализации наивный подход без технических сложностей.

В данной версии в основе используется технологии:
* Guice DI
* Netty Server, для обработки запросов.

Обработчики запросов созданы stateless для экономии времени/памяти, данные пользователя хранятся в контесте соеданения.

Если игроку не находится пара сразу, то его данные отправляются в потокобезопасное хранилище,
пока не появится игрок, составящий ему пару.

Данный подход примечателен тем, что он почти не использует архитектурные паттерны,
работает быстро, не требует памяти и в дополнение к тому потоко безопасен.

Из минусов можно выделить крайнюю сложность тестирования, код не устойчив к изменениям.
